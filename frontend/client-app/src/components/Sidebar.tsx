...
export default function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { icon: CreditCard, labelKey: 'transactions', href: '/transactions' },
    { icon: Rocket, labelKey: 'goals', href: '/goals' },
    { icon: Sparkles, labelKey: 'insights', href: '/insights' },
    { icon: ShoppingBag, labelKey: 'aiShop', href: '/ai-shop' },
    { icon: BookOpen, labelKey: 'financialTips', href: '/financial-tips' },
    { icon: GalleryVerticalEnd, labelKey: 'showcase', href: '/showcase' },
  ];

  return (
    <aside
      className={`hidden md:flex fixed left-0 top-0 h-full bg-white border-r border-zinc-200/70 transition-all duration-200 z-50 flex flex-col ${
    ...
    >
      ...
      <div className="p-2.5 border-t border-zinc-200/70">
        ...
        >
          ...
          {!isCollapsed && <span className="text-sm font-medium">{t('onboarding')}</span>}
        </Link>
        <button
        ...
        >
          {!isCollapsed && <span className="text-sm font-medium ml-2">{t('collapse')}</span>}
