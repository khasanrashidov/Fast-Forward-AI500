  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {rates.map((rate) => (
        <div
          key={rate.currency}
          className="bg-white/70 backdrop-blur-xl border border-white/40 rounded-2xl p-4 shadow-sm flex flex-col items-center justify-center text-center"
        >
          ...
          <p
            className={`text-[10px] font-medium ${rate.isUp ? 'text-[var(--primary)]' : 'text-red-500'}`}
          >
            {rate.change}
          </p>
        </div>
